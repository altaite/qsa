...
public class PdbLine {
	...
	public PdbLine(String line) {
		...
		if (recordName.startsWith("HETATM")) {
			...
			throw new RuntimeException("PdbLine can be used only on ATOM or" + " HETATM lines.");
		}
	...
	public String getPdbString() {
		...
		if (null != temperatureFactor_) {
			printRight(String.format(locale_, "%6.2f", temperatureFactor_), 61, 66, sb);
		}
		if (null != segmentId_) {
	...
	public void setResidueId(ResidueId ri) {
		this.residueId_ = ri;
		this.chainId_ = ri.getChain().getId().charAt(0);
		this.residueSequenceNumber_ = Integer.toString(ri.getSequenceNumber());
